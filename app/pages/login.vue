...
<template>
  <form class="w-full max-w-sm rounded-2xl border border-line bg-panel/90 p-6 shadow-2xl" @submit.prevent="submit">
    ...
    <p class="mt-1 text-sm text-mist">Email and password for your library.</p>

    <label class="mt-6 block text-xs uppercase tracking-wider text-mist">Email</label>
    <input
      v-model="email"
      type="email"
      required
    ...
    <p class="mt-4 text-center text-sm text-mist">
      New here?
      <NuxtLink to="/register" class="text-gold hover:underline">Create an account</NuxtLink>
    </p>
  </form>
</template>
